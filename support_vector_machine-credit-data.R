...
########################################################################################
library(e1071)
Classifying <- svm(formula = default ~ ., data = trainning, type = 'C-classification',
                   kernel = 'radial', cost = 5)
Classifying
predictions <- predict(Classifying, newdata = test[-4])
confusion_matrix <- table(test[,4], predictions)
confusion_matrix
