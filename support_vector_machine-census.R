...
#########################################################################################
library(e1071)
Classifying <- svm(formula = income ~ ., data = trainning, type = 'C-classification',
                   kernel = 'radial', cost = 5)
Classifying
predictions <- predict(Classifying, newdata = test[-15])
confusion_matrix <- table(test[,15], predictions)
confusion_matrix
